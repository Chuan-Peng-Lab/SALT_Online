...
jsPsych.plugins["psychophysics"] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    class polygon_stimulus extends visual_stimulus {
      constructor(stim) {
        super(stim);

        if (typeof this.line_length === 'undefined') alert('You have to specify the line_length of the fixation cross.');
        if (typeof this.line_sides === 'undefined' || this.line_sides <= 2) alert('You should specify the sides of the polygon');
      ...
      show(){
        ...
        // draw
        ctx.strokeStyle = this.line_color;
        let n = this.line_sides;
        let r = this.radius;
        let xc1,yc1;
        for (let i = 0; i < n + 1; i++) {
          xc1 = this.currentX + Math.cos((2 * i * Math.PI) / n) * r;
          // console.log(xc);
          yc1 = this.currentY + Math.sin((2 * i * Math.PI) / n) * r;
          // console.log(yc);
          ctx.lineTo(xc1, yc1);
        }
        ctx.stroke();
